...
@task
def test_ecs():
    r = run_task(['172.66.110.12 ansible_user=root ansible_port=22 ansible_ssh_pass=******'], 'setup')
    print(r)
    return r
    ...
    try:
        ...
        for instance in ecs_list:
            if 'PrivateIp' in instance:
                ...
                #     ansible_hosts.append('%s ansible_user=root ansible_port=22 ansible_ssh_pass=******' % '172.66.110.122')
                # else:
                ansible_hosts.append('%s ansible_user=root ansible_port=22 ansible_ssh_pass=******' % instance['PrivateIp'])
            else:
